...
import (
	...
	usercases "github.com/nurana88/online-shopping/Domain/Usercases"
)

// Register user

func RegisterUser(res http.ResponseWriter, req *http.Request) {
	var user usercases.User
	...
	result, saveErr := usercases.CreateUser(user)
	if saveErr != nil {
		log.Println(saveErr)
...
func LoginUser(res http.ResponseWriter, req *http.Request) {
	var user usercases.LoginDetails
	//res.Header().Set("Content-Type", "json/application")
	err := req.ParseForm()
	if err != nil {
		log.Fatal("Error in parsing request:", err)
	...
	result, saveErr := usercases.GetUser(user)
	if saveErr != nil {
		log.Println(saveErr)
		res.Write([]byte("Bad request 400"))
		//res.WriteHeader(http.StatusBadRequest)
